                            ...
                            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                                <div class="p-6">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        @if($booking->statut == 5 && is_null($booking->etat_restitution_id))
                                            <button class="hs-dropdown-toggle w-full flex items-center justify-between px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                                    type="button"
                                                ...
                                                <span class="flex items-center">
                                                  ...
                                                  Ajouter État Restitution
                                                </span>
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                                                </svg>
                                            </button>
                                        @elseif($booking->statut < 5 || $booking->statut == 7)
                                            <button class="hs-dropdown-toggle w-full flex items-center justify-between px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                                    type="button"
                                                    data-hs-overlay="#hs-terminer-location-modal">
                                                <span class="flex items-center">
                                                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        @else
                                            <button class="hs-dropdown-toggle w-full flex items-center justify-between px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                                    type="button"
                                                    data-hs-overlay="#hs-archiver-location-modal">
                                                <span class="flex items-center">
                                                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                                                  </svg>
                                        ...
                                        @if($booking->statut == 7 || $booking->statut == 1)
            ...
            function fetchPannes() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        // Sélectionner le select où les pannes seront ajoutées
                        const $selectPannes = $('#id_etat_vehicule');
                        $selectPannes.empty();

                        $('#id_etat_vehicule').append('<option value="">Sélectionnez un etat</option>');

                        $.each(data, function(key, panne) {
                            var dateObj = new Date(panne.date);
                            var formattedDate = formatDate(dateObj);

                            $('#id_etat_vehicule').append('<option value="' + panne.id + '"> Etat du ' + formattedDate + '</option>');
                        });
